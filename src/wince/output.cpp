...
openLogFile(const wchar_t *vn)
{
    ...
    outputLogfile = CreateFile(fn, GENERIC_WRITE, FILE_SHARE_READ,
...
flushLogFile()
{
    Output("enter flushLogFile()");
    if (outputLogfile)
        FlushFileBuffers(outputLogfile);
    Output("leave flushLogFile()");
}
...
void
fnprepare(const wchar_t *ifn, wchar_t *ofn, int ofn_max)
{
    // Don't translate absolute file names
    if (ifn[0] == L'\\')
        wcsncpy(ofn, ifn, ofn_max);
    else
        _snwprintf(ofn, ofn_max, L"%s""%s", SourcePath, ifn);
}
...
preparePath()
{
    ...
    //wchar_t sp[200];
    GetModuleFileName(hInst, SourcePath, ARRAY_SIZE(SourcePath));
...
//    wchar_t fn[200];
//    fnprepare(L"earlyharetlog.txt", fn, sizeof(fn));
//    FILE *logfd = _wfopen(fn, L"r");
//    if (logfd) {
//        // Requesting early logs..
//        fclose(logfd);
//        openLogFile(L"haretlog.txt");
//    }
    openLogFile(L"haretlog.txt");
...
cmd_log(const char *cmd, const char *args)
{
    ...
    wchar_t wvn[MAX_CMDLEN];
    mbstowcs(wvn, vn, ARRAY_SIZE(wvn));
    int ret = openLogFile(wvn);
    if (ret)
